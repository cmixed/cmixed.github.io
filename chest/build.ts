...
const outDir = join(__dirname, '..', 'dist', 'chest');
...
function renderFilters(categories: string[]): string {
  const allBtn = `<button class="chest-filter active" data-category="">全部</button>`;
  const catBtns = categories
    .map(
      (cat) =>
        `<button class="chest-filter" data-category="${escapeXml(cat)}">${CATEGORY_ICONS[cat] || ''} ${CATEGORY_NAMES[cat] || cat}</button>`
    )
    .join('');
...
function renderResources(resources: ResourceData[]): string {
  if (resources.length === 0) {
    return `<div class="chest-empty"><div class="chest-empty-icon">📦</div><p>暂无资源</p></div>`;
  }
  ...
  return resources
    .map((r) => {
      ...
      const fileBtn = r.file
        ? `<a href="files/${encodeURIComponent(r.id)}/${encodeURIComponent(r.file)}" class="chest-download" download>下载 ${escapeXml(r.file.split('.').pop() || '')}</a>`
        : `<span class="chest-download" style="opacity:0.5;cursor:not-allowed">文件缺失</span>`;
      const metaParts = [];
      if (r.size) metaParts.push(escapeXml(r.size));
      if (r.date) metaParts.push(escapeXml(r.date));
      const metaStr = metaParts.join(' · ');

      return `<article class="chest-card fade-in-item" data-category="${escapeXml(r.category)}">
            <div class="chest-card-header">
                <div class="chest-card-title">${escapeXml(r.title)}</div>
                <span class="chest-card-category">${icon} ${catName}</span>
            </div>
            <div class="chest-card-desc">${escapeXml(r.description)}</div>
            <div class="chest-card-tags">${tags}</div>
            <div class="chest-card-meta">
                <span class="chest-card-info">${metaStr}</span>
                ${fileBtn}
            </div>
        </article>`;
    })
